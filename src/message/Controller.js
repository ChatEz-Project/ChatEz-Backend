...
const sendMessage = async (req, res) => {
  try {
    ...
    let fileUrl = null;
    if (req.file != null) {
      fileUrl = await FirebaseStorageConnector.uploadFileToFirebase(
        req.file,
        sender
      );
...
const getMessagesForSidebar = async (req, res) => {
  try {
    ...
    console.log(`Getting messages for ${client}`);
    const messages =
      await MessageConnector.getLatestMessageFromEachConversation(client);
    return res.status(200).send(messages);
  } catch (err) {
    console.error(err);
    return res.status(500).send(`Server Error: ${err}`);
...
const getMessagesForFriend = async (req, res) => {
  try {
    ...
    const messages = await MessageConnector.getFriendMessages(
      clientEmail,
      friendEmail
    );
    return res.status(200).send(messages);
...
const setFriendMessagesToRead = async (clientEmail, friendEmail) => {
  try {
    console.log(
      `Getting setting messages from ${friendEmail} to read by ${clientEmail}`
    );
    return await MessageConnector.setFriendMessageRead(
      clientEmail,
      friendEmail
    );
  } catch (err) {
